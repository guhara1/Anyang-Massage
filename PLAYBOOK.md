...
> 간다GO 지역 SEO 템플릿의 제작 방법론입니다. 이 구조를 복제해
> 다른 지역(예: 남양주, 하남, 의정부)에도 동일한 사이트를 만들 수 있습니다.
> 핵심 설계: **시 → (행정구) → 대표 행정동 → 지하철역**, 페이지 수를 과하게 늘리지 않는 린(lean) 구조.
>
> **이 저장소(안양)는 행정구가 둘(만안구·동안구)인 경우의 적용 예입니다.** 원본 템플릿은
> 행정구가 없는 시(구리)를 기준으로 작성되었으므로, 아래 본문의 "행정구가 없는 시" 설명은
> 안양에서는 **시 → 행정구 허브 2개 → 대표 행정동 → 지하철역**으로 한 단계 확장됩니다
> (`content/districts.py` 가 행정구 허브 페이지를 담당). 번호 동 통합·역 1페이지·중복 방지 원칙은 동일합니다.
